...
class adLDAPUsers {
    ...
    * @return array
    */
    public function all_users($includeDescription = false, $search = "*", $sorted = true)
    {
        if (!$this->adldap->getLdapBind()) { return false; }
        
        // Perform the search and grab all their details
        $filter = "(&(objectClass=user)(samaccounttype=" . adLDAP::ADLDAP_NORMAL_ACCOUNT .")(objectCategory=person)(cn=" . $search . "))";
        $fields = array("samaccountname","displayname","givenname","userprincipalname","sn");
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
        ...
        for ($i=0; $i<$entries["count"]; $i++){
            ...
            if ($includeDescription && strlen($entries[$i]["displayname"][0])>0){
				...
				$user = array('givenname'=>$entries[$i]["givenname"][0],'sn'=>$entries[$i]["sn"][0],'userprincipalname'=>$entries[$i]["userprincipalname"][0]);
                $usersArray[] = $user;
